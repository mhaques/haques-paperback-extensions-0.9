  ...
  var URLBuilder = class {
    ...
    formatObjectQuery(key, value) {
      return Object.entries(value)
        .map(([objKey, objValue]) =>
          objValue !== void 0 ? `${key}[${objKey}]=${objValue}` : void 0,
        )
        .filter((x) => x !== void 0);
    }
  ...
  var MangabuddyExtension = class {
    ...
    async getSearchResults(query, metadata) {
      ...
      const getFilterValue = (id) =>
        query.filters.find((filter4) => filter4.id == id)?.value;
      ...
      $2(".list.manga-list .book-detailed-item").each((_, element) => {
        ...
        if (
          genres2.length > 0 &&
          typeof query.filters.find((filter4) => filter4.id == "genres")
            ?.value === "object"
        ) {
          const filterGenres = query.filters.find(
            (filter4) => filter4.id == "genres",
          )?.value;
          const hasExcluded = genres2.some(
    ...
    async getMangaDetails(mangaId) {
      ...
      const image =
        $2(".img-cover img").attr("data-src") ||
        $2(".img-cover img").attr("src") ||
        "";
      ...
      let status = "UNKNOWN";
      const statusText = $2("p strong:contains('Status')")
        .next("a")
        .text()
        .toLowerCase();
    ...
    async getChapterDetails(chapter) {
      ...
      try {
        ...
        const pages = [];
        const scriptContent =
          $2('script:contains("var chapImages")').html() || "";
